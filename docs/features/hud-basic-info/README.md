...
- [x] Logs: a missing panel, gauge or button bitmap warns and the HUD is skipped or the button left out, never a swallowed error. **Deviation:** an unmapped `var id` is a *trace* event, not a warning. The plan said warn-once, but the server sends about twenty of them on every login — the combat stats, the fourth-job stats, the skill point — and none is a fault. Twenty warnings per login would train everyone to ignore the log. `--trace=status` shows them, once per id.
