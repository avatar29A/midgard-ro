...
func TestToggleWindowOpensAndCloses(t *testing.T) {
	...
	if open := b.ToggleWindow(WindowInfo); !open {
...
func TestWindowsToggleIndependently(t *testing.T) {
	...
	for _, w := range []HUDWindow{WindowInfo, WindowSkill, WindowItem, WindowMap} {
		b.ToggleWindow(w)
	}
	...
	for _, w := range []HUDWindow{WindowInfo, WindowSkill, WindowMap} {
		if !b.IsWindowOpen(w) {
...
func TestDescribeHUDListsInStripOrder(t *testing.T) {
	...
	if got, want := b.describeHUD(), "Status, Skill, Map"; got != want {
